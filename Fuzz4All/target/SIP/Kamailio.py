...
class KamailioTarget(Target):
    ...
    def validate_individual(self, file_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            s.settimeout(3)
            s.sendto(payload.encode("utf-8"), ("127.0.0.1", 5060))
            data, _ = s.recvfrom(1024)
            if data:
                return True, f"Received response: {data[:50].decode(errors='ignore')}"
            else:
                return False, "No response from Kamailio"
        except Exception as e:
            return False, f"SIP error: {str(e)}"
